...
Im Gegensatz zu klassischen SaaS-Lösungen läuft die gesamte schwere Logik — Selenium-Scraping, Webseiten-Crawl, Kontakt-Extraktion und LLM-Aufrufe — **lokal auf der Maschine des Nutzers**. Der Cloud-Anteil ist ein dünnes Gateway zur Stammdaten-Synchronisation und für operatorseitig bezahlte Drittdienste (z. B. valueSERP). Diese Architektur ist bewusst gewählt: keine fremden Server, die Recherche-Anfragen mitlesen, keine Cloud-Quotas auf Selenium-Threads, kein Wartungsaufwand bei Lastspitzen.

## Was AVA tut

Pro Firma fanned die Pipeline auf 6 spezialisierte Producer aus, die sich gegenseitig anstoßen:
...
| `website` | Strukturdaten | Beste Treffer-Webseite (valueSERP + LLM-Judge) |
...
Status pro Firma × pro Stage liegt live als Matrix im Desktop, mit Drilldown auf Producer-Logs und Selenium-Screenshots im Fehlerfall.
...
- **CRM-Anbindung** per OAuth (HubSpot live, Salesforce + Dynamics in Vorbereitung). Tokens liegen verschlüsselt im OS-Schlüsselbund
...
Aktuell **v0.1.58**. Pre-1.0 — die Architektur ist stabil, Featureflächen wachsen pro Release. Das Projekt befindet sich in einer aktiven §8-Transition von einem reinen Cloud-Stack hin zur hier beschriebenen Compute-on-User-Architektur. Migrations-Status liegt in [`DECISIONS.md`](./DECISIONS.md) und [`INVENTORY.md`](./INVENTORY.md).

## Installation

Vorgefertigte Builds: [Releases](https://github.com/eproX-GmbH/ava-services/releases). Aktuell signiert + notarisiert für **macOS arm64** (Apple Silicon). Windows ist im CI deaktiviert; das Workflow-File ist intakt — `if: false` umlegen, wenn der Pilot Windows-Nutzer braucht.
...
_Fragen, Feedback, Bugs:_ [hello@pattern-paradise.shop](mailto:hello@pattern-paradise.shop)

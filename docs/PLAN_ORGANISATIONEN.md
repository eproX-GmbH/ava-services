...
| O5 ✅ | Desktop v0.1.544: Schluesselquelle je Anbieter (`keySource` eigen/organisation, Default Organisation ohne eigenen Schluessel), Chat-Aufrufe ueber `GATEWAY/v1/llm/<anbieter>` mit Nutzer-JWT (ai-provider: baseURL+headers fuer alle Anbieter), Producer per `AVA_LLM_VIA_GATEWAY=1` + `PRODUCER_GATEWAY_TOKEN`, Apify im Contact-Producer per `AVA_APIFY_VIA_GATEWAY=1`; providerLock: Anbieter/Modelle/Schluessel gesperrt, Chat-/Producer-Modell aus der Vorgabe, Einstellungen read-only mit Hinweis; Organisations-Seite verwaltet Organisationsschluessel (Admin); Chat-Tool settings_set_key_source. Watchlist/Personen-Radar im Hauptprozess: seit v0.1.580 ebenfalls Organisationsschluessel ueber `/v1/proxy/apify` (ApifyAccess, eigener Token gewinnt ausser unter Sperre); Limit-Banner kam mit O6 (429) | 3 Tage | – |
| O6 ✅ | 2026-09-05: Gateway TenantQuota (off | org_total Monatsbudget | per_user_daily Tagesbudget, US-Cent, hardStop), Vorabpruefung im Stellvertreter-Proxy (429 org_quota_exceeded mit retry-after; ohne hardStop Header x-ava-org-quota), Routen GET/PUT /v1/tenants/me/quota, GET /v1/tenants/me/usage (Admins alle, Mitglieder eigener). Desktop v0.1.549: Banner bei 429 (Chat zeigt deutsche Meldung, Hinweis „mit eigenem Schluessel geht es weiter"), Organisations-Seite: Limits (Admin) + Verbrauch je Mitglied/Tag, Chat-Tools org_limits_set/org_usage. Eigene Schluessel bleiben unlimitiert (Entscheidung 3). **O6b (2026-09-07, Desktop v0.1.570):** Kanaltrennung — Chat sendet `x-ava-llm-channel: chat`, Producer/Hauptprozess-Jobs zaehlen als `background` (Default ohne Header); TenantQuota.split + chatOrgMonthlyCents/chatUserDailyCents, LlmUsage.channel (Migration 20260907_quota_channel); 429 traegt `channel`; GET quota liefert `standChat`; usage mit Chat/Hintergrund-Anteilen. Aufgebrauchtes Hintergrund-Budget stoppt den Chat nicht mehr | 2 Tage | ja |
| O7 ✅ | 2026-09-05: Datenfluss-Tabelle (docs/PLAN_COMPLIANCE_ENTERPRISE.md §0) um Stellvertreter-Proxy, Organisationsschluessel und Organisationsvorgaben ergaenzt; Website-Prompt (docs/WEBSITE_PROMPT_COMPLIANCE.md) §5 „Zentrale Schluessel fuer Teams" und §7 Live-Stand Organisationen/Vorgaben/Limits, „in Arbeit" = Teilen (O8/O9), Server-Worker, Dynamics | 0,5 Tag | – |
...
| O9 ✅ | 2026-09-06 (v0.1.551): Gateway `POST /v1/tenants/me/shares/radar` (Batch, unbekannte discoveryIds uebersprungen), Freigabe-Liste mit Kandidatendaten aus DiscoveredCompany; Desktop: Bulk-Button „Mit Organisation teilen" im Radar, Block „Von der Organisation geteilt" (ohne ICP-Filter; Importieren/Ignorieren/Ausblenden, gesehen-Markierung beim Oeffnen), Sammel-Meldung „N neue Firmen aus der Organisation im Radar" im 10-Minuten-Abgleich (einmal je Freigabe, Merker org-shares-notified.json), Chat-Tool radar_share. Tests (PGlite): Batch/Dedup/unbekannt, Kandidatendaten, Ausblenden bleibt bei Reaktivierung | 2 Tage |

...
| `buying_center_freigeben` | Sicht fuer ein Organisationsmitglied erteilen oder entziehen (E-Mail, Name oder Kennung; ohne Angabe: Liste) | ja — es gibt Einschaetzungen ueber Menschen weiter |
| `buying_center_geteilt` | Was Kollegen mir freigegeben haben, je Firma mit Eigentuemer, Anlass, Stand | nein |
...
| BC7 | gebaut (2026-09-20): Gateway `GET/POST /buying-center/{id}/freigaben`, `DELETE …/freigaben/{actorId}` (nur Eigentuemer, nur an Mitglieder der eigenen Organisation), `GET /buying-center-geteilt?companyId=` (eigener Pfad, kollidiert nie mit `{id}`; nur innerhalb der Organisation, Eigentuemer als E-Mail/Name aus `TenantMember`). Werkzeuge `buying_center_freigeben` (Rueckfrage, Mitglied per E-Mail/Name/Kennung ueber `findeMitglied`, mehrdeutig → Auswahl) und `buying_center_geteilt`; `buying_center_anzeigen` findet ohne eigenes Buying Center ein freigegebenes. Desktop: Bereich "Mit dir geteilte Buying Center" (`/buying-center/geteilt`, Firmen aufklappbar → je Kollege ein Eintrag) und Nur-Lese-Bildschirm (`/buying-center/geteilt/:id`, Kopf nennt den Eigentuemer, dieselbe Karte; Verschieben wird nicht gespeichert); Navigationseintrag unter "Firmen" nur mit mindestens einer Freigabe und nur mit Schalter `buyingcenter`. Auslieferung: Gateway-Deploy + naechstes Desktop-Release. |
| Protokoll | 2026-09-21: `BuyingCenterLauf` — eine Zeile je Hintergrundlauf (Entwurf, CRM-Abgleich, Website-Abgleich, Nachfrage, Watchlist, Verknüpfung, Status, Freigabe); `GET/POST …/verlauf` (Desktop meldet nur crm-abgleich und watchlist); Abschnitt „Verlauf“ unter der Karte, `verlauf` in `buying_center_anzeigen`/`_anlegen`. Ausserdem 2026-09-21: Karte mit Zoom/Ziehen/Dialog/Legende (v0.1.708–710, 713), Ausgeschiedene gefiltert + Beschäftigungsbeginn (v0.1.711), „A beeinflusst B“ → Kante sofort. |
| Website-Personentexte | 2026-09-22 (v0.1.721): Der Personen-Extraktor liefert je Person zusaetzlich `seit` (nur wenn die Seite ausdruecklich sagt, seit wann jemand bei DIESER Firma ist; "JJJJ"/"JJJJ-MM") und `beschreibung` (ein Satz, was die Person bei der Firma tut, aus dem Text an der Person). Gilt fuer jede Art von Seite (Team, Ueber-uns, Geschaeftsfuehrung, Presse, Impressum), ohne zusaetzlichen Modellaufruf oder Seitenabruf — zwei Felder mehr im bestehenden Schema. Gateway: `seit` → Fakt `employmentSince` (wie Apify; Startdatum wird nur ergaenzt), `beschreibung` → neuer Fakt `websiteBeschreibung` (10–300 Zeichen, Seite in Observation.evidenceUrl). Buying Center: `beschreibung` je Mitglied, in der Personenkarte sichtbar und in `buying_center_anzeigen`; Rollen-Vorschlag `ava:website` aus der Beschreibung (`vorschlaegeAusBeschreibung`: dieselben Regeln wie beim Titel, aber ganze Woerter und ohne Rueckfall "Anwender"; nur wenn der Titel nichts Konkretes ergab; beim Anlegen, beim Aufnehmen, beim Oeffnen und in der Recherche-Auswertung). Der Hinweis "neu in der Position" greift damit auch ohne Apify. Altbestand: ein erneuter Kontaktlauf (Recherche im Buying Center) liefert nach. |
| Register im Entwurf | 2026-09-22 (v0.1.722): Befund — der Entwurf las nur den Kontakt-Bestand (Deckel 25 nach Personen-ID, ohne Rang); eingetragene Geschaeftsfuehrer (structured-content `ManagingDirector`) und Gesellschafter (master-data Gesellschafterliste) fehlten ganz, wenn sie nicht auf der Website standen (Freiburg: 4 Register-GF fehlten, 2 "Managing Director"-Kontakte am Deckel abgeschnitten). Jetzt: `lib/buying-center-register.ts` laedt GF (immer) und natuerliche Gesellschafter der aktuellen Liste (nur mit Orga-Feature `verflechtungen`, ueber `/api/germany/v1/companies/{id}/shareholders`, Fehler still); Entwurf nimmt Register-Personen zuerst (Kontakt-Person per `gleicherName` verknuepft, jetzt mit Zweitnamen-Toleranz "Michael Erwin Basler" = "Michael Basler"), dann Kontakte nach `rangDesTitels` (Geschaeftsleitung → Leitung/Prokura/Einkauf → Fachkraft → ohne Titel; Ausgeschiedene raus), Deckel 25 gesamt. Vorschlaege `ava:register` (`vorschlaegeAusRegister`: GF → E + Einfluss H; Gesellschafter → R, > 50 % zusaetzlich E). `registerNachziehen` beim Oeffnen (`GET …/vorschlaege`, eigenes) und in der Recherche-Auswertung nimmt fehlende Register-Personen in bestehende Buying Center auf (Lauf `register-abgleich`). Duplikatschutz: `POST …/mitglieder` weist gleiche Namen mit 409 ab bzw. verknuepft ein freies Register-Mitglied mit der Kontakt-Person statt zu verdoppeln. Karte: `kraftLayout` mit Ringen — innen E/R/Register, mittlerer Ring B/S/EK, aussen N/GK/unbekannt (`ringFuer`, Feder auf den Ringradius, Ellipse 1,6:1); „Neu anordnen" nutzt dieselbe Logik. Prokuristen bleiben offen (Rollen-Spiegel sendet nur GESCHAEFTSFUEHRER). |

...
- v0.1.611: workflow_save/update normalisieren Agent-Eingaben (type „tool:<name>“ aus dem Katalog, tool/args/condition/prompt auf Node-Ebene, Argumente ohne args-Huelle); Tool-Node ohne Tool-Name wird NICHT gespeichert, sondern mit klarer Meldung abgelehnt; Tool-Schema mit Beispielen je Node-Typ.
- v0.1.612: Lauf-Historie wie n8n: Seite Workflows → Alle Laeufe (Filter Status/Workflow/Zeitraum, Klick oeffnet den Lauf im Editor via ?lauf=), Lauf-Kopfzeile im Editor (Ausloeser, Firma, Dauer, Status, Warten-Stand, Abbrechen, Wiederholen), je Node Reiter Eingabe | Ausgabe | Details (Items als Tabelle/JSON, Durchgaenge bei passivem Warten, Dauer, Fehler, Hinweise, Platzhalter). IPC executionsAll/rerun.
- v0.1.613: Eigentliche Ursache der leeren Nodes: defineTool.parseArgs validiert mit stripUnknown, und shapeloses yup.object() verliert dabei ALLE Schluessel — parameters, args, connections kamen leer an. Workflow-Tools nutzen jetzt freiesObjekt() (mixed, nicht gestrippt); Regressionstest.
- v0.1.615: Nodes ohne Eingabe-Items (Filter ohne Treffer) laufen nicht: Tool-/KI-Node ueberspringen mit Hinweis statt Aufruf mit leeren Argumenten ("decisions must have at least 1 items"); Warten-Node ohne Items wartet nicht. Lauf endet erfolgreich mit 0 Firmen.
- v0.1.619: Laeufe tragen einen Snapshot der Definition (Nodes + Kanten); alte Laeufe zeigen ihren damaligen Stand (historische Ansicht, schreibgeschuetzt, „Zur aktuellen Version“). Node-Typ im Schritt-Panel per Auswahl wechselbar (Tool → Tool behaelt Argumente).
- v0.1.620: Tool-Ergebnis → Items robust: falscher outputPath faellt auf bekannte Listen zurueck, einziges Array-Feld wird genutzt, leere Listen und Fallbacks stehen als Hinweis am Node (Details-Reiter). Tool-Doku: outputPath nur bei Bedarf.
- v0.1.622 (Operator-Entscheidung): S-Stufen-Gate — workflow_save/workflow_update/workflow_from_conversation nur mit Chat-Modell der Stufe S (docs/MODEL_TIERS.md, tierForModel). Sonst klare Meldung im Chat (Modell wechseln oder im Editor anlegen); Starten, Freigaben, Vorlagen, Lesen und der Editor bleiben frei (workflows/modellstufe.ts).
- v0.1.625: Firmen-Kontext je Item: Laeufe ohne feste Firma (flacher Workflow nach Warten-Node/Import) bauen fuer KI- und perItem-Tool-Nodes den Kontext der Item-Firma (companyId/discoveryId im Item) — $company, Klartext im System-Prompt, Platzhalter je Firma (Cache). Vorher „Keine Angaben verfuegbar“ im Telegram-Bericht. Editor: Wartestand-Badge am Warten-Node, Summen ueber Durchgaenge (v0.1.624). Vorgangs-Meldung: Selenium-Fehler lesbar; Publikations-Producer meldet stale element als voruebergehend.

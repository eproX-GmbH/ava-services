...
| S4 | gebaut (v0.1.728): Eingabezeile, Bilder (bis 4) und Dokumente (PDF/Excel/CSV, bis 3, ueber `parseAttachment`/`composePromptWithAttachments`) als Anhang; Zuege aus dem Sprachmodus tragen `quelle: "sprache"` und stehen im Chatverlauf als "Im Sprachmodus gesprochen"; Banner/Hinweise der App sind waehrend des Gespraechs verborgen (body.sprachmodus) |
| S4b | gebaut (v0.1.728): Ruhezustand nach `ruheSekunden` (Verbindung zu), Countdown-Ring, Wecken per Kugel/Leertaste/Tippen, Aktivierungswort `lib/wachwort.ts` (Pegel-Detektor + Whisper-Fenster), Signalton, Kontext-Uebergabe; Audio-Puffer waehrend des Aufbaus (`PufferAufnahme`: lokal aufnehmen, mit Whisper transkribieren, als Text nachreichen) |
...
| S6 | gebaut (v0.1.728): Push-to-Talk bei stummem Mikrofon (Leertaste halten), Fehlerbilder mit Ursache/Hilfe/Knoepfen (Mikrofon verweigert, kein Mikrofon, Netz/Proxy, Schluessel, Kontingent; `fehlerEinordnen`), Doku docs/SPRACHMODUS.md |

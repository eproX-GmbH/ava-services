...
export function buildSystemPrompt(registry: ToolRegistry): string {
  const persona = [
    ...
    "- Chain tools when needed (e.g. search → get → publications) but do not",
    "  call more than necessary; each call is visible to the user.",
    "- After tools run, summarise the result for the user — don't dump raw",
    "  JSON unless they explicitly ask for it.",
    "",

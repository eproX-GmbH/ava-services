...
export function buildSystemPrompt(
  ...
  const persona = [
    ...
    "  `resolve_import_matches` aufrufen und `matched` + `unmatched` aus dem",
    "  Dry-Run 1:1 durchreichen — das zeigt dem Nutzer ALLE Zweifelsfälle in",
    "  EINER scrollbaren Karte (Kandidaten + Überspringen). Du bekommst",
    "  `commitCompanies` zurück (fertige {name, city}-Liste).",

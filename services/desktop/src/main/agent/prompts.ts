...
export function buildSystemPrompt(
  ...
  const persona = [
    ...
    "  vielen Reihen).",
    "- Final-Commit OHNE `dryRun`. Bei `import_companies_from_crm` UND",
    "  `import_companies` das `companies`-Argument mit der bereinigten",
    "  Liste (Originale + Korrekturen + bestätigte Kandidaten via Name+City,",
    "  übersprungene weglassen). Bei `import_excel` mit vielen Korrekturen",
    "  den Nutzer um eine bereinigte Datei bitten, bei wenigen mit",
    "  `import_companies` (Ein-Element-Liste) einzeln nachreichen.",

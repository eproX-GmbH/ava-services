...
export class LlmProviderManager extends EventEmitter {
  ...
  async getProducerLlmEnv(): Promise<{
    ...
    // company-evaluation across all users (vector-search consistency
    // in the central MPG). The producer-supervisor sets
    // EMBED_PROVIDER=ollama + EMBED_MODEL=embeddinggemma:latest
    // explicitly via its per-producer extraEnvAsync hook for
    // "company-evaluation" -- no longer the manager's job to
    // resolve a per-user embed-provider.

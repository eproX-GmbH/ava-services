...
export const LOGIC_NODES: WorkflowCatalogEntry[] = [
  ...
  { type: "wait", label: "Warten", category: "Ablauf", summary: "Wartet eine Zeitspanne (max 7 Tage) ODER passiv auf einen Vorgang (Import): Firmen gehen einzeln an die Folge-Schritte, sobald ihre Stufen fertig sind; der Lauf pausiert ohne Zeitlimit (Status „wartet“, ueberlebt Neustarts). Ausgabe: ein Item je Firma mit companyId, state (completed/failed), fehlgeschlageneStufen, transactionId.", parameters: { type: "object", properties: { minutes: { type: "number", minimum: 1, maximum: 10080 }, transactionId: { type: "string", description: "Vorgangs-ID, z. B. {{ $json.transactionId }} aus discovery_decide/import — wartet bis die geforderten Stufen aller Firmen im Endzustand sind; danach ein Item je Firma" }, bis: { type: "array", items: { type: "string", enum: ["masterData", "structuredContent", "companyPublication", "website", "companyProfile", "companyContact", "companyEvaluation"] }, description: "Stufen, auf die gewartet wird. Leer = automatisch aus den Folge-Schritten (z. B. $kassenbestand → Jahresabschluesse), mindestens Firmenprofil." }, maxDays: { type: "number", default: 14, description: "Sicherheitsnetz: spaetestens nach so vielen Tagen geht es mit den fertigen Firmen weiter" } } }, write: false, actionKind: "read", costClass: "frei" },

...
//
// Auth: `window.api.auth.getAccessToken()` is awaited per request. The
// main process refreshes the token if it's near expiry, so callers here
...
// we pass the bearer as `?access_token=…` — the gateway's auth middleware
// must accept this on the SSE route. Long-term, switch to
// `@microsoft/fetch-event-source` (uses fetch streaming, supports headers).
// Tracked as a Step 6 follow-up.
//
// Returns a teardown function. Call it from the component's effect cleanup.
export async function gatewaySSE(
  path: string,
